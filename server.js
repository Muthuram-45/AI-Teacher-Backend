...
app.get('/room-status/:roomName', (req, res) => {
  ...
  console.log(`🔍 Room status check: ${roomName} → ${ended ? 'ENDED' : 'ACTIVE'}`);
  res.json({ ended });
});
...
app.post('/encourage-student', async (req, res) => {
  try {
    ...
    const prompt = `You are an encouraging teacher's assistant. A student named "${name}" just asked this academic question: "${question}".
    Provide a very short, one-sentence encouraging response to make the student feel good about asking.
    Examples: "That's a brilliant question, ${name}!", "Excellent observation, ${name}, let's look into it.", "Great curiosity, ${name}!"
    
    RULES:
    1. Keep it under 15 words.
    2. Be positive and use the student's name.
    3. Return ONLY the encouraging statement.`;
    ...
    const completion = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You provide short, professional, and encouraging feedback to students.',
        },
        {
          role: 'user',
          content: prompt,
    ...
    const encouragement = completion.choices[0]?.message?.content.trim() || `Good question, ${name}!`;
